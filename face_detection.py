...
def build_argparser():
    ...
    args.add_argument("--no_show", default=False, help="Optional. Don't show output", action='store_true')
...
def main():
    ...
    if is_async_mode:
        ...
        # frame = np.asarray(frame) ### chnage for FRCNN
    ...
    while cap.isOpened():
        ...
        if is_async_mode:
            ...
            # next_frame = np.asarray(next_frame)  ### chnage for FRCNN
            ret, next_frame = cap.read()
        ...
        if exec_net.requests[cur_request_id].wait(-1) == 0:
            ...
            for i , obj in enumerate(boxes):
                ...
                if obj[4] > args.prob_threshold:
                    width_ratio = w/frame_w
                    height_ratio = h/frame_h
                    xmin = int(obj[0]/width_ratio )
                    ymin = int(obj[1]/height_ratio )
                    xmax = int(obj[2]/width_ratio )
                    ymax = int(obj[3]/height_ratio )
                    label = labels[i]
...
##-----------------MODIFIED- YOGI ---------------------------------------------------------------
                    ...
                    # if (xmax-xmin) > 10 and (ymax-ymin) >10:
                        ...
                        # det_label = labels_map[class_id] if labels_map else str(class_id)
                        cv2.putText(frame,  'Person' + str(round(obj[4] * 100, 1)) + ' %', (xmin, ymin - 7),
        ...
        if writer is None:
            ...
            writer = cv2.VideoWriter('Person_openVino.mp4', fourcc, 30,
